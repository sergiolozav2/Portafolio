  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogTrigger asChild>
        <button
          type="button"
          className="group flex w-full cursor-pointer flex-col rounded-lg border border-transparent bg-transparent text-left transition-colors focus:outline-hidden focus-visible:ring-2 focus-visible:ring-amber-400/70 focus-visible:ring-offset-2 focus-visible:ring-offset-stone-900"
        >
          <ProjectPreviewImage
            images={images}
            title={title}
      ...
      <DialogContent
        ...
        showCloseButton
      >
        <div className="grid h-full items-center gap-6 overflow-y-auto md:grid-cols-[1.5fr_1fr] md:gap-x-8">
            ...
            <div className="flex gap-3 overflow-x-auto">
              {images.map((image, index) => (
                <button
                  ...
                  className={cn(
                    "relative h-20 w-28 shrink-0 cursor-pointer overflow-hidden rounded-md border transition-all duration-300 focus:outline-hidden focus-visible:ring-2 focus-visible:ring-amber-400/80",
                    index === safeActiveIndex
                      ? "border-amber-400/80 shadow-[0_0_0_2px_rgba(250,204,21,0.25)]"
...
function GalleryArrow({ direction, onClick, disabled }: GalleryArrowProps) {
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        "absolute top-1/2 z-10 flex h-10 w-10 -translate-y-1/2 cursor-pointer items-center justify-center rounded-full border border-stone-700/60 bg-stone-900/80 text-stone-100 transition hover:bg-stone-800/80 focus:outline-hidden focus-visible:ring-2 focus-visible:ring-amber-400/70",
        alignment,
